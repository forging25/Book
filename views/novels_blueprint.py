...
from Book.fetcher.cache import cache_owllook_novels_content, cache_owllook_novels_chapter, \
    cache_owllook_search_ranking
from Book.fetcher.function import get_time, get_netloc
from Book.fetcher.novels_tools import get_novels_info
from Book.utils import ver_question
...
async def index(request):
    ...
    if user:
        return template('index.html', title='网络小说搜索引擎', is_login=1, user=user,
                        search_ranking=search_ranking[:25])
    else:
        return template('index.html', title='网络小说搜索引擎', is_login=0, search_ranking=search_ranking[:25])
